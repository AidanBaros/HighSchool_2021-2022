...
void combat() {
	...
	while (PlrHealth >= 0 && MonHealth >= 0) {
		...
		if (MonsterBlock == 1) {
			cout << "You tryed to " << input << " but the monster blocked." << endl;
			MonsterBlock = 0;
		}

		else if (input.compare("slash")) {
			if (MonCls == 1) {
				int crit = rand() % 5;
				MonHealth = MonHealth -= (Attack + crit);
		...
		else if (input.compare("Health Potion") && whichattack > 2) {
			PlrHealth = rand() % 15 + 5;
		}

		MONAT = rand() % MonAT + 1;
		int MAttack = rand() % 10 + 1;
		if (ShieldActive == 1) {
