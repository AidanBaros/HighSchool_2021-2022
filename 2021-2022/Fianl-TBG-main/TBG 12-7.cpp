...
void combat() {
	...
	while (PlrHealth >= 0 && MonHealth >= 0) {
		...
		if (input.compare("Slash")) {
			if (MonCls == 1) {
				int crit = rand() % 5;
				MonHealth = MonHealth -= (PlayerWeapon.BaseAttack + crit);
			}
			else {
		...
		MONAT = rand() % MonAT + 1;
		if (MONAT == 1) {
			cout << "The monster slashs at you.";
		}
		else if (MONAT == 2) {
			cout << "The monster jabs at you.";
		}
		else if (MONAT == 3) {
			cout << "The monster strikes at you.";
		}
		else if (MONAT == 4) {
			cout << "The monster blocks";
		}
	}
}
